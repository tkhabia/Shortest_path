...
void Uint512ToStream(unsigned int st ,unsigned int en  , uint512 &data ,hls::stream< ap_uint<32> , q_size> &qu ){
		#pragma hls pipeline
	...
	void load_Queue(uint512 *queue , unsigned int st , unsigned int end , hls::stream< ap_uint<32> , q_size> &qu ){
		unsigned int s1 = st/16  , en1 = 16, en= end/16;
		if ( s1 * 16 + 16 > end)
			en1 = end%16;
	#pragma HLS DATAFLOW
		uint512 temp1 = queue[s1%max_nodes]  ;
		Uint512ToStream(st%16 , en1 , temp1 ,qu) ;
		Ib_load:for (unsigned int i = s1+1 ; i<en ; i++ ){
...
#pragma HLS DATAFLOW
        ...
        WritingData:
            for (unsigned int i = st;i < lend; i++){
                #pragma HLS PIPELINE 
                // reading data 
                parent = qu.read(); 
